...
public class ModsPackLister {
  ...
  public void connectionSever(ClientCustomPacketEvent event) {
      String md5sString = "";
      for (String md5 : AntiAntiCheat.md5List) {
      ...
      ((FMLEmbeddedChannel)AntiAntiCheat.channelmap.get(Side.CLIENT)).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.TOSERVER);
      ((FMLEmbeddedChannel)AntiAntiCheat.channelmap.get(Side.CLIENT)).writeOutbound(new Object[] { new ModPackImpl(md5sString, secretKey, ivSpec) });
  }
}
